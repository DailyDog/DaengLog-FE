...
class PetProfileProvider extends ChangeNotifier {
  ...
  PetData? get currentPet =>
      _currentPetId != null ? _petsCache[_currentPetId!] : null;
  PetData? get defaultPet =>
      _defaultPetId != null ? _petsCache[_defaultPetId!] : null;
        ...
        if (_petsCache.containsKey(pet.id)) {
          ...
          _petsCache[pet.id] = _petsCache[pet.id]!.copyWith(
            ...
            profileImageUrl: _sanitizeUrl(pet.profileImageUrl) ??
                _petsCache[pet.id]!.profileImageUrl,
            age: pet.age,
  ...
  Future<Map<String, dynamic>?> loadPetDetail(int petId) async {
    ...
    try {
      ...
      print(
          "🔵 Detail API 응답: ${detail['name']} - ${detail['profileImageUrl']}");
  ...
  void updatePet(
    ...
    if (_petsCache.containsKey(petId)) {
      _petsCache[petId] = _petsCache[petId]!.copyWith(
        name: name,
        profileImageUrl:
            profileImageUrl != null ? _sanitizeUrl(profileImageUrl) : null,
        birthday: birthday,
        gender: gender,
        species: species,
...
// 프로필 이미지 업데이트
  Future<bool> updatePetImage(int petId, XFile imageFile) async {
    print('🔵 updatePetImage 시작 - petId: $petId, file: ${imageFile.path}');

    try {
      final newImageUrl =
          await PetImageUpdateApi().updatePetImage(petId, imageFile);
      print('🔵 API 응답: $newImageUrl');
      ...
      print(
          '🔴 updatePetImage 실패 - newImageUrl: $newImageUrl, cache contains: ${_petsCache.containsKey(petId)}');
      return false;
    } catch (e) {
