...
class _ChatCommunicationScreenState extends State<ChatCommunicationScreen> {
  ...
  final DiaryCreatePromptApi _diaryCreatePromptApi =
      DiaryCreatePromptApi(); // API 인스턴스
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 1,
        toolbarHeight:
            MediaQuery.of(context).size.height * 0.075, // 화면 높이의 7.5%
        centerTitle: true,
        title: Text(
      ...
      body: ListView.builder(
        ...
        itemCount:
            _messages.length + 1 + (_loading ? 1 : 0), // 날짜 + 메시지 개수 + 로딩 박스
        itemBuilder: (context, idx) {
          if (idx == 0) {
            ...
            return Padding(
              padding: EdgeInsets.only(
                  bottom:
                      MediaQuery.of(context).size.height * 0.02), // 화면 높이의 2%
              child: Center(
                child: Container(
                  padding: EdgeInsets.symmetric(
                    horizontal:
                        MediaQuery.of(context).size.width * 0.05, // 화면 너비의 5%
                    vertical:
                        MediaQuery.of(context).size.height * 0.01, // 화면 높이의 1%
                  ),
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight: FontWeight.bold,
                      fontSize:
                          MediaQuery.of(context).size.width * 0.04, // 화면 너비의 4%
                    ),
                  ),
                ),
              ),
          ...
          if (_loading && msgIdx == _messages.length) {
            // 로딩 중이면 로딩 박스 표시
